...
def feild_selection(ctx: typer.Context, table_name: str, req_value: str):
    ...
    if logged_in:
        ...
        if req_value == "year":
            ...
            product = typer.prompt("What's the product you are looking for?")
            year = typer.prompt("What's the year?")
            dayofyear = typer.prompt("What's the day_of_year?")
            input_values = {"product": str(product),
                            "year": str(year),
                            "day_of_year": str(dayofyear)}
        elif req_value == "file":
            product = typer.prompt("What's the product you are looking for?")
            year = typer.prompt("What's the year?")
            dayofyear = typer.prompt("What's the day_of_year?")
...
@app.command()
def s3_bucket(ctx: typer.Context,search_method, src_bucket, dest_bucket, dest_folder, prefix: Optional[str] = typer.Argument(None), file_name: Optional[str] = typer.Argument(None)):
    ...
    if search_method == 'fieldselection':
        search_method = "Field Selection"
    elif search_method == "filename":
        search_method = "File Name"
    else:
        typer.echo("Search Method can only be within this two options fieldselection|filename")
